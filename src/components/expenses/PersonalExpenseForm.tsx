...
export const PersonalExpenseForm: React.FC<PersonalExpenseFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!description.trim()) {
      setError('Keterangan wajib diisi (misal: Makan siang)');
      return;
    }
    ...
    try {
      ...
      setError('Gagal menyimpan pengeluaran pribadi');
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
        ...
        <div>
          <h4 className="text-sm font-bold text-slate-800">Catat Pengeluaran Pribadi</h4>
          <p className="text-[11px] text-slate-500">
            Dipotong dari Jatah Hidup; selisih memotong Tabungan Pribadi
          </p>
        </div>
      </div>
